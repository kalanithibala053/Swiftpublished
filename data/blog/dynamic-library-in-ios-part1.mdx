---
title: 'Dynamic Library in iOS (Part 1)'
date: '2019-10-11'
tags: ['frameworks--libraries']
draft: false
summary: 'Are you Duplicating a piece of code for a particular functionality across multiple Apps? Do you find it difficult to fix a bug in all those places? If Yes, then Dynamic Library is an answer to your problems.'
---
...
![Breaking Bad van in the desert](/images/articles/iOS/framework/dynamic/breaking-bad.jpeg)
<p class="imageCredits">Breaking Bad Meth Van, Courtesy: <a href="https://www.artstation.com/artwork/XBmkGw">Artstation</a></p>
...
> Heisenberg: Please check out [Static Library in iOS](https://swiftpublished.com/article/static-library-in-ios-part1) about Library & Static Linking before you continue.

> Jesse: Okay, got it. Now, How Dynamic Library is different from it’s Static counterpart?

> Heisenberg: The key differences between Static & Dynamic Libraries (Dylib) are as discussed below
...
![Flow diagram of static linking](/images/articles/iOS/framework/dynamic/static-linking.png)
<p class="imageCredits">App using static libraries, Courtesy: <a href="https://developer.apple.com/library/archive/documentation/DeveloperTools/Conceptual/DynamicLibraries/100-Articles/OverviewOfDynamicLibraries.html">Apple</a></p>
...
![Flow diagram of dynamic linking](/images/articles/iOS/framework/dynamic/dynamic-linking.png)
<p class="imageCredits">App using dynamic libraries, Courtesy: <a href="https://developer.apple.com/library/archive/documentation/DeveloperTools/Conceptual/DynamicLibraries/100-Articles/OverviewOfDynamicLibraries.html">Apple</a></p>

![Flow diagram explaining dynamic linking](/images/articles/iOS/framework/dynamic/dynamic-linking-explained.png)
<p class="imageCredits">Flow diagram explaining dynamic linking</p>
...
![Flow diagram explaining static vs dynamic load](/images/articles/iOS/framework/dynamic/static-vs-dynamic.png)
<p class="imageCredits">Flow diagram explaining static vs dynamic load</p>
...
![Swift Dynamic libaries in Finder](/images/articles/iOS/framework/dynamic/core-swift-dylib.png)
<p class="imageCredits">Swift Dynamic libaries in Finder</p>

2. Helper Swift libraries, used by *swiftc* (Swift Compiler) & Xcode are in *User Library*

![Swift helper libaries in Finder](/images/articles/iOS/framework/dynamic/helper-swift-lib.png)
<p class="imageCredits">Swift helper libaries in Finder</p>
...
![Xcode Dylib print libraries](/images/articles/iOS/framework/dynamic/dylib-print-libraries.png)
<p class="imageCredits">Xcode Dylib print libraries Settings</p>

It will give an output like this for an empty single view iOS App on Debugger console:

![Xcode Dylib print libraries output](/images/articles/iOS/framework/dynamic/dylib-print-libraries-output.png)
<p class="imageCredits">Xcode Dylib print libraries output</p>
...
### Launch Time
- When you tap on that App Icon in your iPhone, app binary gets loaded onto memory along with the dynamic libraries required only for showing the launch screen & then the first screen.
- It’s the time between the `main` function call of the executable & the `return true` from `didFinishLaunchingWithOptions` of the **AppDelegate**.
- App state transitions from *terminated* to *active* during this time.

### Run Time
- It’s the time when the app is actually running & user is interacting with it, dynamic libraries can also be loaded during this time based on the need of a particular module.
- It’s post Launch Time, the time between the `return true` from `didFinishLaunchingWithOptions` & the `applicationWillTerminate` of the **AppDelegate**.
- App state transitions from *active* to *terminated* post this time.
